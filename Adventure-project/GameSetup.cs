...
public class GameSetup
{
    ...
    public Rooms CreateWorld()
    {
        ...
        Room secret = new Room("Secret", "This room is encrypted. Decrypt it to see its contents.")
        {
            IsEncrypted = true,
            EncryptedFilePath = "room_secret.enc",
            RoomId = "secret"
        };

        Room treasure = new Room("Treasure", "This room is encrypted. Decrypt it to see its contents.")
        {
            IsEncrypted = true,
            EncryptedFilePath = "room_treasure.enc",
            RoomId = "treasure"
        };
        ...
        deeper.AddExit(Directions.East, secret);

        // Connect rooms - Win room
        win.AddExit(Directions.South, deeper);

        // Connect encrypted rooms
        secret.AddExit(Directions.West, deeper);
        secret.AddExit(Directions.North, treasure);
        treasure.AddExit(Directions.South, secret);
